...
def pad_sentences(sents, pad_id):
    ...
    #TODO 
    '''
    1. Find the longest sentence in sents 
    2. Pad every other sentence to be the same legnth as longest
        -iterate over each sentence
        -if sentence is shorter than LONGEST
        -add [pad] / 0 / pad_id until sentence equals length of LONGEST
    3. return augmented sents
    '''
    #return empty list of sents is empty
    if not sents: 
        return []

    #Find and store longest sentence in sents
    longest = max(len(sent) for sent in sents)

    #Pad every other sentence to be as long as longest sentence
    aug_sents = sents 
    for sent in aug_sents: 
        while len(sent) < longest: 
            sent.append(pad_id) #is this the correct pad or do we do 0, or "<pad>"?
...
def main():
    ...
    # Reduce LR when dev accuracy plateaus.
    ...
    for epoch in range(args.max_train_epoch):
        for batch in data_iter(train_data, batch_size=args.batch_size, shuffle=True):
            ...
            if train_iter % args.eval_niter == 0:
                ...
                # Step the scheduler based on dev accuracy.
